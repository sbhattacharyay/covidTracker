...
jhuData <- read.csv(file = lastFile) %>% mutate(Last.Update=as.character(Last.Update)) %>% mutate(Last.Update=parse_datetime(Last.Update,format="%Y-%m-%dT%H:%M:%S",locale = locale(tz="GMT"))) %>% mutate(Source = "Johns Hopkins University CSSE")
...
sk_table<-select(sk_table,Province.State,Confirmed,Recovered,Deaths,Source)
...
covidData <- covidData[!(covidData$Country.Region=="Italy" & covidData$Source=="Johns Hopkins University CSSE") & !(covidData$Country.Region=="Korea, South" & covidData$Source=="Johns Hopkins University CSSE"),] %>% mutate(Active = Confirmed - Recovered - Deaths)
...
col_order <- c("Country.Region", "Province.State", "Last.Update","Confirmed","Recovered","Deaths","Active","New.Cases","New.Deaths","New.Recovered","Advisory.Level","Advisory.DateUpdated","Latitude","Longitude","Source")
