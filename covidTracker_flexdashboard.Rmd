...
v <- reactiveValues(data = covidData)
w <- reactiveValues(labs = covidData)

observeEvent(input$countryChoice!="World", {
  v$data <- covidData[covidData$Country.Region==input$countryChoice & covidData[,input$trackVar]>0,]
  w$labs <- labs[covidData$Country.Region==input$countryChoice & covidData[,input$trackVar]>0]
})
...
map <- renderLeaflet({
  leaflet(data = v$data,options = leafletOptions(minZoom = 2, maxZoom = 8)) %>% setView(lng = -92 ,lat=26,zoom = 3) %>% addProviderTiles(provider = providers$CartoDB.Positron)%>%setMaxBounds(lng1 = -180,lat1 = -90,lng2 = 180,lat2 = 90)%>% addCircleMarkers(radius=~log(Active),lng = ~Longitude,lat = ~Latitude,weight = ~(Active)**(1/3),label = lapply(w$labs, htmltools::HTML),labelOptions = labelOptions(style = list("font-weight" = "normal", padding = "3px 8px"),textsize = "15px", direction = "auto")) 
})
leafletOutput('map')
