...
italy_pdfs <- sort(list.files(path_to_italy_data,"*.pdf",full.names=T))
italy_xlsx <- sort(list.files(path_to_italy_data,"*.xlsx",full.names=T))
...
italy_table <- read.xlsx(italy_xlsx,sheetIndex=1,startRow=4)%>% select(c(1,6,7,8,5))%>% rename(Province.State=1,Recovered=2,Deaths=3,Confirmed=4,Active=5) %>% filter(Confirmed != "") %>% head(-1)%>% mutate(Admin2="",Last.Update=parse_datetime(italy_dateTim_updated,format="%d %B %Y, ore %H.%M",locale = locale("it",tz="Europe/Rome")),Source="Ministry of Health of Italy")
