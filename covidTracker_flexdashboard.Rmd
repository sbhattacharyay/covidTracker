...
Packages <- c("tidyverse","shiny","magrittr","httr","plotly","rvest","tabulizer","XML","xml2","htmltab","pdftools","tesseract","leaflet","openxlsx","htmltools","shinydashboard")
...
reactive_case_count <- renderText({
  paste0(prettyNum(sum(covidData$Confirmed), big.mark=","), " cases")
})

reactive_recovered_count <- renderText({
  paste0(prettyNum(sum(covidData$Recovered), big.mark=","), " recovered")
})

reactive_active_count <- renderText({
  paste0(prettyNum(sum(covidData$Active), big.mark=","), " active cases")
})

reactive_country_count <- renderText({
  paste0(length(unique(covidData$Country.Region)), " countries/regions affected")
})

absolutePanel(id = "controls", class = "panel panel-default",fixed=TRUE, height = "auto",                                   h3(textOutput("reactive_case_count"), align = "right"),
...
renderPlotly({
  ggplot(data=ts_confirmed(), aes(x=as.character(Country.Region),y=value)) +
  geom_line()+
  geom_point()+
